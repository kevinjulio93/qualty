...
import { Autocomplete, Button, Dialog, DialogActions, DialogContent, DialogContentText, DialogTitle, FormControl, InputLabel, MenuItem, Paper, Select, Stack, TextField, Typography } from "@mui/material";
...
import { createWinerie, getAllWineries, getWinerie, updateWinerie } from "../../services/winerie.service";
...
function Winerie () {
    //code
    const [winerie,setWinerie]=useState({_id:"",name:"",type:"",inventory:[],associated_winery:null});
    ...
    const handleRemoveAction=(index)=>{
        setListItemSelected([...listItemSelected.filter((item,indexItem)=>indexItem!==index)]);
        setWinerie({...winerie,inventory:[...winerie.inventory.filter((item,indexItem)=>indexItem!==index)]});
        const itemFound=listItemSelected.find((item,indexItem)=>indexItem===index);
        if(winerie.type===typesWineries.SECUNDARIA){
            if(itemFound){
    ...
    const resetChanges=()=>{
        setCurrentItem(null);
        setWinerie({_id:"",name:"",type:"",associated_winery:null,inventory:[]});
        setListItemSelected([]);
    }

    const saveWinerie=async (e)=>{
        e.preventDefault();
        const winerieFound=wineries.find((win)=>win.name.trim().toLowerCase()===winerie.name.trim().toLowerCase());
        if(!winerieFound){
            delete winerie._id;
    ...
    const decrementAmount=(amountAdd,idItem)=>{
        ...
        const itemFound=listItemsWinerieMain.find((data)=>data.item._id===idItem);
        const index=listItemsWinerieMain.indexOf(itemFound);
        listItemsWinerieMain[index].amount=listItemsWinerieMain[index].amount-amountAdd;
    ...
    const handleAddAction = async() => {
        if(currentItem.amount > 0){
            ...
            setOpen(false);
            setWinerie({...winerie,inventory:[...winerie.inventory,{amount:currentItem.amount,item:currentItem._id}]});
    ...
    const handlerOpenDialog=(item?:any)=>{
        const itemFound=listItemSelected.find((dataItem)=>dataItem._id===item._id);
        if(!itemFound){
            setCurrentItem({...item,amount:0});
            setOpen(!open);
        }else{
            setMessageDialog("Este articulo ya existe en la tabla de articulos agregados")
            handlerOpenDialogMessage();
        }
    }
    ...
    const handlerOpenDialogAddAmountItem=(data?:any)=>{
        ...
        const itemRepet=listItemSelected.find((dataItem)=>dataItem._id===data.item._id);
        if(itemRepet){
            setMessageDialog("Este articulo ya existe en la tabla de articulos agregados")
            handlerOpenDialogMessage();
            return;
    ...
    const searchItemInventoryWinerieMain=(value:string)=>{
        ...
        const filter=listItemsInventory.filter((data)=>data.item.name.toLowerCase().includes(value.trim()) || data.item.code===value.trim() );
            ...
            <section className='winerie-container'>
                <header className="assistance-container__actions">
                    <div className="content-page-title">
                        <Typography variant="h5" className="page-header">{params.winerieId!==undefined ? "Editar bodega":"Crear bodega" }</Typography>
                        <span className="page-subtitle">{params.winerieId !==undefined ? "Editar bodega y añadir nuevos articulos.":"Crear bodega y añadir articulos."}</span>
                    </div>
                </header>
                ...
                <Paper elevation={1} className="assistance-container__form-section">
                    ...
                    />
                        <FormControl sx={{width: 300}}>
                            {
                                ...
                                <Autocomplete
                                    ...
                                    readOnly
                                    value={winerie?.type || ""}
                                    renderInput={(params) => <TextField {...params} label="Tipo de bodega" />}
                       ...
                       {winerie.type===typesWineries.PRINCIPAL || winerie.type=== "" || winerie.type=== null ?  "":
                        params.winerieId!==undefined ? 
                        <Autocomplete
                            disablePortal
                            id="combo-box-demo"
                            ...
                            readOnly
                            sx={{ width: 300 }}
                            renderInput={(params) => <TextField {...params} label="Asociar bodega" />}
                    ...
                    : winerie.type===typesWineries.SECUNDARIA && itemsWinerieMain.length >0 ? <>
                        {
                            isLoadingItems ?
                            <LoadingComponent></LoadingComponent>
                            :
                            ...
                            <Search
                                ...
                                voidInputFunction={()=>setItemsWinerieMain([...auxItemsWinerieMain])}
                            />
                            </div>
                            <Table>
                            <TableRow header>
                            ...
                            <TableRow key={index}>
                                ...
                                <Stack className="actions-cell" direction="row" spacing={2}>
                                    <AddCircleIcon
                                    className="action-item-icon action-item-icon-add"
                                    onClick={() => handlerOpenDialogAddAmountItem(data)}
                                    ></AddCircleIcon>
                                </Stack>
                                </TableCell>
                            </TableRow>
                        ...
                        <div className="assistance-container__form-section__table">
                            <div className="panel-heading"> 
                                Listado de items (articulos) agregados de la bodega asociada
                                <div>
                                </div>
                            </div>
                            <Table>
                ...
                <Dialog open={open} >
                        ...
                        <DialogContentText>
                            {itemSeletedWinerieMain && itemSeletedWinerieMain.amount >0 ? `Cantidad disponible : ${itemSeletedWinerieMain.amount}`:""}
                        </DialogContentText>
                        <br></br>
                        <TextField
                            autoFocus
